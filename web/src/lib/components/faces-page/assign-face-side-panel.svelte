  ...
  <div class="flex place-items-center justify-between gap-2">
    {#if !searchFaces}
      <div class="flex items-center gap-2">
        ...
        <p class="flex text-lg text-immich-fg dark:text-immich-dark-fg">Sélectionner des personnes</p>
      </div>
      <div class="flex justify-end gap-2">
        ...
        {#if !isShowLoadingNewPerson}
          <CircleIconButton icon={mdiPlus} title="Créer une nouvelle personne" on:click={handleCreatePerson} />
        {:else}
    ...
    {:else}
      ...
      <CircleIconButton icon={mdiClose} title="Annuler la recherche" on:click={() => (searchFaces = false)} />
    {/if}
  </div>
  <div class="px-4 py-4 text-sm">
    <h2 class="mb-8 mt-4 uppercase">Tout le monde</h2>
