<script lang="ts">
  ...
  const toggleFavorite = async () => {
    try {
      ...
      handleError(
        error,
        `Impossible de ${asset.isFavorite ? 'ajouter la ressource aux' : 'retirer la ressource des'} favoris`,
      );
    }
  };
  ...
  const toggleArchive = async () => {
    try {
      ...
      handleError(
        error,
        `Impossible de ${asset.isArchived ? 'ajouter la ressource à' : 'retirer la ressource de'} l'archive`,
      );
    }
  };
