...
<div id="media-type-selection">
  <fieldset>
    <legend class="immich-form-label">TYPE DE MEDIA</legend>
    <div class="flex flex-wrap gap-x-5 gap-y-2 mt-1">
      <RadioButton name="media-type" id="type-all" bind:group={filteredMedia} label="TOUT" value={MediaType.All} />
      <RadioButton name="media-type" id="type-image" bind:group={filteredMedia} label="Image" value={MediaType.Image} />
      <RadioButton name="media-type" id="type-video" bind:group={filteredMedia} label="Vidéo" value={MediaType.Video} />
    </div>
  </fieldset>
</div>
