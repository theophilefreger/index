...
<fieldset>
  <legend class="immich-form-label">Type de recherche</legend>
  <div class="flex flex-wrap gap-x-5 gap-y-2 mt-1 mb-2">
    ...
    <RadioButton
      ...
      id="file-name-radio"
      bind:group={selectedOption}
      label="Contexte du fichier (nom du dossier/projet ou nom du fichier)"
...
{#if selectedOption === TextSearchOptions.Context}
  <label for="context-input" class="immich-form-label">Recherche par IA</label>
  <input
  ...
  />
{:else}
  <label for="file-name-input" class="immich-form-label"
    >Contexte du fichier (nom du dossier/projet ou nom du fichier)</label
  >
  <input
    class="immich-form-input hover:cursor-text w-full !mt-1"
    type="text"
