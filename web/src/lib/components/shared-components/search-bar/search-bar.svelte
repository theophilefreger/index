...
<div class="w-full relative" use:clickOutside={{ onOutclick: onFocusOut }} use:focusOutside={{ onFocusOut }}>
  ...
  >
    ...
    <label for="main-search-bar" class="sr-only">Recherche intelligente</label>
    <input
      type="text"
      name="q"
      id="main-search-bar"
    ...
    <div class="absolute inset-y-0 {showClearIcon ? 'right-14' : 'right-2'} flex items-center pl-6 transition-all">
      <CircleIconButton title="Voir les filtres" icon={mdiTune} on:click={onFilterClick} size="20" />
    </div>
    {#if showClearIcon}
      <div class="absolute inset-y-0 right-0 flex items-center pr-2">
        <CircleIconButton type="reset" icon={mdiClose} title="Effacer" size="20" />
      </div>
    {/if}
