<script lang="ts">
  ...
  const handlePasswordSubmit = async () => {
    try {
      ...
      title = (sharedLink.album ? sharedLink.album.albumName : $t('public_share')) + ' - Mémoire Vive';
      description = sharedLink.description || `${sharedLink.assets.length} photos et vidéos partagés`;
    } catch (error) {
